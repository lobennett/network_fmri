...
class Runner:
    ...
    def __call__(self, command, **kwargs):
        ...
        if command[0] == "uvx":
            study = Path(command[command.index("-d") + 1])
            campaign = study / ".mechababs/campaigns/campaign"
            campaign.mkdir(parents=True)
            (campaign / "pyproject.toml").write_text(
                '[tool.uv.sources]\n'
                'mechababs = { git = "https://github.com/con/mechababs.git", rev = "' + 'd' * 40 + '" }\n'
                'babs = { git = "https://github.com/PennLINC/babs.git", rev = "' + 'e' * 40 + '" }\n'
            )
...
def test_initialize_creates_study_metadata_raw_slot_campaign_and_sibling(tmp_path):
    ...
    init = next(command for command in commands if command[0] == "uvx")
    assert "campaign" in init and "init" in init
    assert "git+https://github.com/con/mechababs.git@" + "d" * 40 in init
    assert "https://github.com/PennLINC/babs.git@" + "e" * 40 in init
    assert any(command[-2:] == ("--sourcedata", "sourcedata/raw") for command in commands)
...
def test_existing_campaign_rejects_changed_tool_pins(tmp_path):
    config = mechababs(tmp_path)
    manager = StudyManager(config, raw_dataset(tmp_path), tmp_path / "license.txt", runner=Runner())
    manager.initialize(subjects=("s01",))
    path = config.campaign_dir / "pyproject.toml"
