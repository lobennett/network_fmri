...
_USAGE = """usage:
  ...
  network_fmri qa-motion --cohort C -- ...     motion+behavioral exclusions (MRIQC IQMs) -> lev1
  network_fmri qa-lev1 --cohort C -- ...       add lev1 outliers -> lev2
...
def main(argv: list[str] | None = None) -> int:
    ...
    if verb == "qa-reject":
        ...
        return f(rest)
    if verb == "qa-motion":
        from network_fmri.qa.exclusions import motion as f

        return f(rest)
    if verb == "qa-lev1":
        from network_fmri.qa.exclusions import lev1 as f
    ...
    if verb == "validate":
        ...
        return f(rest)
    if verb == "check":
        from network_fmri.qa.check import main as f

        return f(rest)
    if verb == "mriqc-iqms":
        from network_fmri.qa.mriqc import record as f
