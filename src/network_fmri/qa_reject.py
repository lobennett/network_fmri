...
from network_fmri.acquisitions import NON_FUNC, QA_REJECT

MARKER = "_qa-reject"


def suffix_labels(suffix: str) -> set[str]:
    """Acquisition labels that map to a given BIDS suffix (e.g. T1w -> MPRAGE label)."""
    return {label for label, e in NON_FUNC.items() if e.get("suffix") == suffix}
